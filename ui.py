...
from internetarchive import configure, upload, download, session
...
class first:
    ...
    def home(self):
        ...
        tk.Label(self.root1, text="logged in mail: " + ename, font=(
    ...
    def downloadclicked(self):
        ...
        #self.staus_label = tk.Label(self.root1, textvariable=self.progressbar,text="Speed: ", font=('calibre', 10, 'bold'))
        ...
        self.staus_label.place(x=100,y=100)
    ...
    def next(self):
        ...
        item = internetarchive.ArchiveSession()
        a=(item.get_metadata(self.down))
        a=a["files"]
        b=[]

        for i in a:
            try:
                b.append([i["name"],int(i["size"])])
            except:
                 b.append([i["name"],0])
        ...
        try:
            ...
            pass    
        for i in b:
            print(f"https://archive.org/download/{self.down}/{i[0].replace(' ', '%20')}")
            self.downloader1(f"https://archive.org/download/{self.down}/{i[0].replace(' ', '%20')}"  , filename=f"{self.down}/{i[0]}", downloading=f"{self.down}/{i[0]}.dow")
            
      
        #https://archive.org/download/kksdnfkjsdf/New%20Text%20Document.txt
    ...
    def downloader1(self,url,filename,downloading):
        def callback(velocity):
            ...
            self.progressbar.set(f'Status:{velocity}')
            time.sleep(0.2)
        
        downloader(url=url, save_as=filename, downloadingFileName=downloading, fn_callback=callback).downlaod()
